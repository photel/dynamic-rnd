...
class Agent(nn.Module):
    ...
    def get_value(self, x):
        ...
        features = self.extra_layer(cnn_fc)
        value_ext = self.critic_ext(features + cnn_fc)
        value_int = self.critic_int(features + cnn_fc)
...
if __name__ == '__main__':
    ...
    # store the concatenated list of parameters from the agent and RND predictor
    ...
    curiosity_rewards = torch.zeros(rollout_size).to(device)
    ...
    # during this phase take random actions and store the observations to intialise obs_rms, which is used in normalising observations
    next_ob = []
    ...
    for update in range(1, num_updates+1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Get the predicted feature representation of the next state from the fixed target network
        ...
        # Update the curiosity bonus
        curiosity_reward_per_env = np.array(
            [discounted_reward.update(step_reward) for step_reward in curiosity_rewards.cpu().data.numpy().T]
        )
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                # get advantages for extrinsic rewards - uses Generalized Advantage Estimation (GAE) with a lambda param
                ...
                # get advantages for intrinsic rewards
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Setup a tensor of random values in interval [0,1) from a uniform distribution
                ...
                # Final loss is a sum of policy loss, weighted value loss, forward loss between the RND predictor and target nets, less the weighted entropy loss.
